  ...
  <q-page class="q-pa-sm" style="padding-left: 15px; padding-right: 15px">
    <div class="q-pa-md q-gutter-sm">
      <q-banner inline-actions rounded style="background:linear-gradient(to right,#99c9ff, #c6ddff) " class=" text-white">
        <strong>Jadwal Mengajar {{gelardepan+' '+guru+' '+gelarbelakang}} | TA. {{ta+' '+semester}}</strong>
      ...
      >
        ...
        <template v-slot:body-cell-kelas="props">
          <q-td :props="props">
           {{props.row.kelompok_kelas ? props.row.tingkat_kelas+' '+props.row.nama_kelas+' '+props.row.kelompok_kelas : props.row.tingkat_kelas+' '+props.row.nama_kelas}}
          </q-td>
        </template>
      </q-table>
  ...
  export default {
    ...
    methods: {
      ...
      getjadwalgurufull (){
        ...
        this.$axios.get(`getjadwalgurufull/${id}/${ids}`)
      ...
      getDetailGuru (){
        const id = btoa( this.$q.localStorage.getItem('dataUser').id_relasi)
        this.$axios.get(`detailGuru/${id}`)
          .then(response => {

            // console.log(response)
            if (response.data.msg.status === true) {

              this.guru = response.data.data.Nama
              this.gelardepan = response.data.data.Gelar_Depan
              this.gelarbelakang = response.data.data.Gelar_Belakang
