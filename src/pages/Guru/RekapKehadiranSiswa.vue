  ...
  <q-page class="q-pa-sm" style="padding-left: 15px; padding-right: 15px">
    <div class="q-pa-md q-gutter-sm">
      <q-banner inline-actions rounded style="background:linear-gradient(to right,#99c9ff, #c6ddff) " class=" text-white">
        <strong>Rekap Kehadiran Siswa | TA. {{jTA +' '+jSemester+' Kelas '+jKelas}} Mapel {{mapel}}</strong>
  ...
  export default {
    ...
    data() {
      ...
      return {
        waktu: null,
        loading: false,
        mapel: atob(this.$route.params.mapel),
        jTA: splitinfo[0],
        jKelas: kelas,
    ...
    methods: {
      ...
      detailabsen (data){
          ...
          this.$router.push({name: 'guru_detailabsen' , params: {id :ids,namasiswa: nama}})
